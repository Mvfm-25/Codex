...
proof (induction z)
  show "\<forall>x y. add (add x y) 0 = add x (add y 0)" by simp
next
  fix x0::nat
  assume HI:"\<forall>x y. add (add x y) x0 = add x (add y x0)"
  show "\<forall>x y. add (add x y) (Suc x0) = add x (add y (Suc x0))" by (simp add:HI)
qed
...
proof (induction z)
  ...
  proof(rule allI, rule allI)
    ...
    have "add (add x0 y0) 0 = add x0 y0" by (simp only:add01)
    also have "... = add x0 (add y0 0)" by (simp only:add01)
    finally show "add (add x0 y0) 0 = add x0 (add y0 0)" by simp
  qed
next
  fix z0::nat
